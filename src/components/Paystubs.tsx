...
export const Paystubs = () => {
  ...
  const generatePaystubEmail = (paystub: typeof employeePaystubs[0]) => {
    ...
    // Create formatted paystub content
    const paystubContent = `
PAYSTUB - ${paystub.employeeName}
Position: ${paystub.employeeTitle}
Period: ${periodText}

SUMMARY:
========
Total Labor Cost: $${paystub.totalCost.toFixed(2)} (excludes taxes and remittances)
Total Hours: ${paystub.totalHours.toFixed(1)}
${paystub.totalLoaCount > 0 ? `Live Out Allowances: ${paystub.totalLoaCount} × $200 = $${paystub.totalLoaAmount.toFixed(2)}` : ''}
...
${paystub.entries
  ...
  .map(entry =>
    `${formatLocalDate(entry.date)} | ${entry.jobNumber} - ${entry.jobName} | ${entry.hourTypeName} | ${entry.provinceName} | ${entry.hours.toFixed(2)}h @ $${entry.costWage.toFixed(2)}/h${(entry.loaCount || 0) > 0 ? ` + ${entry.loaCount} LOA` : ''} = $${entry.totalCost.toFixed(2)}`
  ).join('\n')}

TOTAL COMPENSATION: $${paystub.totalCost.toFixed(2)}

NOTE: This amount excludes taxes, remittances, and deductions.
Live Out Allowances (LOAs) are shown separately when applicable.

Generated by 4Front Trackity-doo on ${new Date().toLocaleDateString()}
    `.trim();

    // Create email
    const subject = encodeURIComponent(`Paystub - ${paystub.employeeName} (${periodText})`);
    const body = encodeURIComponent(paystubContent);

    // Open email client (will use default, which should be Outlook if set as default)
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-xl text-gray-100 flex items-center gap-2">
                      ...
                      <button
                        onClick={() => generatePaystubEmail(paystub)}
                        className="hover:text-orange-300 hover:underline transition-colors duration-200 text-left flex items-center gap-2"
                        title="Click to generate email with paystub"
                      >
                        {paystub.employeeName}
                        <Mail className="h-4 w-4 text-gray-400 hover:text-orange-300" />
                      </button>
                    </CardTitle>
