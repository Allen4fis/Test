  ...
  const getJobDates = (job: Job): JobDateInfo[] => {
    ...
    return dates.map((date) => {
      ...
      const totalLoaAmount = dayTimeEntries.reduce(
        (sum, entry) => sum + getEntryLoaTotal(entry),
        0,
      );
      const laborCostWithoutLoa = dayTimeEntries.reduce(
        (sum, entry) => sum + getCostWithoutLoa(entry),
        0,
      );
      const laborBillableWithoutLoa = dayTimeEntries.reduce(
        (sum, entry) => sum + getBillableWithoutLoa(entry),
        0,
      );
      const laborCost = laborCostWithoutLoa + totalLoaAmount;
      const laborBillable = laborBillableWithoutLoa + totalLoaAmount;
                              ...
                              <TableCell className="text-gray-100">
                                <div className="space-y-2">
                                  ...
                                  <textarea
                                    ...
                                    className="w-full h-16 text-xs bg-blue-900/20 border border-blue-500/30 rounded p-1 text-blue-200 resize-none cursor-pointer hover:bg-blue-900/30 transition-colors"
                                    title="Click to select all, then Ctrl+C to copy these employees"
                                  />
                                </div>
                              </TableCell>
                              <TableCell className="text-gray-100">
                          ...
                          <TableRow className="border-t-2 border-orange-500/50 font-bold">
                            ...
                            <TableCell className="text-green-400 text-lg">
                              ...
                              {breakdown.timeEntries
                                .reduce(
                                  (sum, entry) =>
                                    sum + getBillableWithoutLoa(entry),
                                  0,
                                )
                                .toFixed(2)}
                            </TableCell>
