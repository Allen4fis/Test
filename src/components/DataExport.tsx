...
export function DataExport() {
  ...
  const filteredRentalEntries = useMemo(() => {
    ...
    return filtered;
  }, [rentalEntries, dateRange, employeeTypeFilter, employees]);
  ...
  const detailedTimeEntries = useMemo(() => {
    return filteredTimeEntries.map((entry) => {
      ...
      return {
        ...
        employeeCategory:
          entry.employeeCategory || employee?.category || "employee",
  ...
  const summary = useMemo(() => {
    ...
    filteredRentalEntries.forEach((rentalEntry) => {
      ...
      // Handle both old format (startDate) and new format (rentalDate)
      const dateToUse = rentalEntry.rentalDate || (rentalEntry as any).startDate;
      if (!dateToUse) return;

      const month = dateToUse.substring(0, 7); // YYYY-MM format
  ...
  const generateCSV = () => {
    ...
    Object.entries(summary.monthlyBreakdown)
      ...
      .forEach(([month, data]) => {
        ...
        const totalCost = data.billableLaborCost + data.rentalCost + data.nonBillableCost;
