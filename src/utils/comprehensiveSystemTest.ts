...
export class ComprehensiveSystemTester {
  ...
  private async testDataIntegrity(): Promise<void> {
    ...
    try {
      ...
      if (!data) {
        ...
        const requiredFields = ['employees', 'jobs', 'timeEntries', 'hourTypes', 'provinces'];
  ...
  private async testMemoryLeaks(): Promise<void> {
    ...
    try {
      ...
      if (memoryDiff > 50 * 1024 * 1024) { // 50MB threshold
        test.warnings?.push(`High memory usage detected: ${memoryDiff / 1024 / 1024}MB`);
      }
  ...
  private async testPerformance(): Promise<void> {
    ...
    try {
      ...
      if (computationTime > 2000) { // 2 second threshold
  ...
  private generateReport(totalTime: number): SystemTestReport {
    ...
    const averageTestTime = performanceTimes.reduce((sum, p) => sum + p.time, 0) / performanceTimes.length;
    const slowestTest = performanceTimes.reduce((prev, curr) => prev.time > curr.time ? prev : curr);
    const fastestTest = performanceTimes.reduce((prev, curr) => prev.time < curr.time ? prev : curr);
