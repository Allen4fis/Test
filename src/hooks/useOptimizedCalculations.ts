...
export function useOptimizedCalculations(): OptimizedCalculations {
  ...
  const generateCacheKey = useCallback((data: any[], type: string): string => {
    const dataString = JSON.stringify({
      ...
      checksum: data.reduce(
        (sum, item) => sum + (item.id?.charCodeAt(0) || 0),
        0,
      ),
    });
  ...
  const isCacheValid = useCallback(
    (key: string, maxAge: number = 30000): boolean => {
  ...
  const getTimeEntrySummaries = useCallback(
    (entries: TimeEntry[]) => {
      const cache = cacheRef.current;
  ...
  const getJobProfitData = useCallback(
    (jobs: Job[], entries: TimeEntry[]) => {
      const cache = cacheRef.current;
      ...
      const profitData = jobs.map((job) => {
        ...
        const profitMargin =
          totals.totalBillable > 0 ? (profit / totals.totalBillable) * 100 : 0;
  ...
  const getEmployeeSummaries = useCallback(
    (employees: Employee[], entries: TimeEntry[]) => {
      const cache = cacheRef.current;
      ...
      const summaries = employees.map((employee) => {
        ...
        const totals = employeeEntries.reduce(
          (acc, entry) => {
            return {
              totalHours: acc.totalHours + entry.hours,
              totalCost:
                acc.totalCost + entry.hours * (entry.costWageUsed || 0),
              totalBillable:
                acc.totalBillable + entry.hours * (entry.billableWageUsed || 0),
        ...
        return {
          ...
          averageHoursPerEntry:
            totals.entryCount > 0 ? totals.totalHours / totals.entryCount : 0,
  ...
  const getCacheStats = useCallback(() => {
    ...
    return {
      ...
      size:
        cacheRef.current.timeEntrySummaries.size +
...
export function useDateRangeCalculations(
  ...
  return useMemo(() => {
    ...
    return {
      ...
      totalCost: filteredEntries.reduce(
        (sum, entry) => sum + entry.hours * (entry.costWageUsed || 0),
        0,
      ),
      totalBillable: filteredEntries.reduce(
        (sum, entry) => sum + entry.hours * (entry.billableWageUsed || 0),
        0,
      ),
    };
