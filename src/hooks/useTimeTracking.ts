...
export function useTimeTracking() {
  ...
  const timeEntrySummaries = useMemo((): TimeEntrySummary[] => {
    return appData.timeEntries.map((entry) => {
      ...
      // Only DSPs and their subordinates are always at regular time rates (1x multiplier)
      ...
      } else if (isDspOrSubordinate) {
        // For subordinates of DSPs, use regular time (1x) multiplier for cost calculation
        totalCost = entry.hours * adjustedCostWage; // Always 1x for DSP team costs
      } else {
  ...
  const summaryByDateAndName = useMemo((): SummaryByDateAndName[] => {
    const grouped = appData.timeEntries.reduce(
      (acc, entry) => {
        ...
        // Only count hours towards daily totals if it's not "Billable" hour type
        if (hourType.name !== "Billable") {
          acc[key].totalHours += entry.hours;
          acc[key].totalEffectiveHours += effectiveHours;
        }
